...
def projectTag(projFile, buildFile, name, mainDir, moduleDir):
	...
	conditionProperty(projFile, includeDebug)
	conditionProperty(projFile, includeDebug)
	importProject(projFile, 'props')
	extensionSettings(projFile)
	propertySheets(projFile, includeDebug) # Include if debug version is included
	propertySheets(projFile, includeDebug)
	userMacros(projFile)
	libPropertyGroup(projFile, includeDebug, mainDir, name) # include if debug version is included
	libPropertyGroup(projFile, includeDebug, mainDir, name)
	itemDefinitionGroup(projFile, includeDebug, moduleDir, mainDir)
	itemDefinitionGroup(projFile, includeDebug, moduleDir, mainDir)
	headerFiles(projFile, moduleDir)
	sourceFiles(projFile, moduleDir)
	importProject(projFile, 'targets') # ??? Why is this done twice?
	extensionSettings(projFile) # This too
...
def conditionProperty(projFile, isDebug):
        if (isDebug):
...
def propertySheets(projFile, isDebug):
        if (isDebug):
...
def libPropertyGroup(projFile, isDebug, mainPath, projName):
        if(isDebug):
...
def itemDefinitionGroup(projFile, isDebug, mainPath, modulePath):
        if (isDebug):
        ...
        clCompile(projFile, isDebug, mainPath, modulePath)
        printAndOutput(projFile, '</ItemDefinitionGroup>')

def clCompile(projFile, isDebug, mainPath, modulePath):
        ...
        additionalIncludeDirectories(projFile, mainPath) # Debug & non are equivalent
        link(projFile, isDebug, mainPath)
        printAndOutput(projFile, '</ClCompile>')

def link(projFile, isDebug, mainPath):
...
def additionalIncludeDirectories(projFile, modulePath):
        ...
        # hwpfilter\win32\hwpfilter\hwpfilter - what is this madness?
        printAndOutput(projFile, modulePath + '\\..\\solver\\410\\wntmsci12.pro\\inc')
        printAndOutput(projFile, modulePath + '\\..\\solver\\410\\wntmsci12.pro\inc\\external')
        printAndOutput(projFile, modulePath + '\\..\\solver\\410\\wntmsci12.pro\\inc\\offuh')
        printAndOutput(projFile, '%(AdditionalIncludeDirectories)')
        printAndOutput(projFile, '</AdditionalIncludeDirectories>')
...
runEveryFolder('C:\steve\TestArea\main')
